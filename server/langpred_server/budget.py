...
def register(req: BudgetRequest, project_id: str = "default") -> BudgetStatus:
    store = get_store()
    record = BudgetRecord(
    ...
    return evaluate(req.trace_id, project_id=project_id) or _initial_status(record)


def evaluate(trace_id: str, project_id: str = "default") -> BudgetStatus | None:
    """Re-evaluate a budget against current trajectory. Returns ``None`` if no
    budget is registered for the trace."""
    store = get_store()
    rec = store.get_budget(trace_id, project_id=project_id)
    if rec is None:
        return None

    traj = get_trajectory(trace_id, project_id=project_id)
    spent = traj.total_usd if traj else 0.0
    pred = get_service().predict(trace_id, "cost", project_id=project_id)
    remaining_p50 = max(0.0, pred.p50 - spent)
    remaining_p90 = max(0.0, pred.p90 - spent)
    remaining_by_quantile = {
        "p50": remaining_p50,
        "p90": remaining_p90,
        "p99": max(0.0, pred.p99 - spent),
    }
    selected_remaining = remaining_by_quantile.get(rec.quantile, remaining_p50)
    ...
    elif (spent + selected_remaining) >= rec.cap_usd:
        breached_now = True
        reason = (
            f"predicted breach: spent ${spent:.4f} + {rec.quantile} remaining "
            f"${selected_remaining:.4f} ≥ cap ${rec.cap_usd:.4f}"
        )
    ...
    return BudgetStatus(
        ...
        quantile=rec.quantile,  # type: ignore[arg-type]
        spent_usd=spent,
...
def status(trace_id: str, project_id: str = "default") -> BudgetStatus | None:
    return evaluate(trace_id, project_id=project_id)
